    ...
    <body>
        ...
        <div class="container mt-5">
            <div class="row">
                ...
                <?php
                $rs = Database::search("SELECT * FROM `order_history` WHERE `user_email`='" . $user["email"] . "'");
                $num = $rs->num_rows;
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        $rs2 = Database::search("SELECT * FROM `order_item` INNER JOIN `stock` ON `order_item`.`stock_stock_id` =`stock`.`stock_id` 
                                            INNER JOIN `product` ON `stock`.`product_id`=`product`.`id` WHERE `order_item`.`order_history_order_h_id`='" . $d["order_h_id"] . "'");
                                        $num2 = $rs2->num_rows;
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td>Rs.<?php echo ($d2["order_item_qty"] * $d2["price"]); ?></td>
                                            </tr>
                                        <?php
                                        }
